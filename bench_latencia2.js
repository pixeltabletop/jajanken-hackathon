    ...
    eq: {
      ...
      items: {
        ...
        properties: {
          ...
          ev: { type: 'array', minItems: 2, maxItems: 2, items: { type: 'integer' } } // [inicio, fin] en la nota
        }
      }
    }
  }
...
ev son las posiciones de caracter [inicio, fin] del fragmento de la nota que justifica la fila.
...
writeFileSync(`bench/latencia2_${MODEL_NAME}.json`, JSON.stringify({ model: MODEL_NAME, variantes: out }, null, 2));
...
try {
  ...
  for (const e of p.eq) {
    const cita = NOTAS[0].slice(e.ev[0], e.ev[1]);
